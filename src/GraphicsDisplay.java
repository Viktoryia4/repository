...
public class GraphicsDisplay extends JPanel {
    ...
    public GraphicsDisplay() {
        ...
        graphicsStroke = new BasicStroke(2.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND, 10.0f, new float[] {2*10,1*10,1*10,1*10,1*10,1*10,1*10,1*10,2*10,1*10,1*10,1*10,2*10}, 0.0f);
...
и Y - сколько пикселов
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        for (Double[] point: graphicsData) {
            ...
            Point2D.Double corner = shiftPoint(center, 13, 13);

            marker.setFrameFromCenter(center, corner);
            canvas.draw(marker);
            canvas.draw(new Line2D.Double(shiftPoint(center,-13,0),shiftPoint(center,13,0)));

            canvas.draw(new Line2D.Double(shiftPoint(center,0,13),shiftPoint(center,0,-13)));
        }
    }
